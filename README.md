...
| 路径 | 类型 | 用途说明 |
|------|------|---------|
| `config/` | 目录 | 各设备型号的编译配置文件，文件名对应设备型号（如 `ufi003.config`） |
| `files/` | 目录 | 编译时覆盖到固件的自定义文件，结构对应固件根目录 |
| `files/etc/` | 目录 | 固件 `/etc` 目录下的自定义配置文件 |
| `files/www/` | 目录 | 固件 `/www` 目录下的自定义 Web 文件 |
| `img/` | 目录 | README 文档中使用的教程截图 |
| `scripts/` | 目录 | 编译过程中执行的辅助脚本 |
| `scripts/show_hardware_info.sh` | 文件 | 显示硬件信息的脚本，编译时用于输出设备硬件参数 |
| `工具与脚本/` | 目录 | 刷机相关工具和辅助脚本合集 |
| `工具与脚本/gen_config.py` | 文件 | 生成设备编译配置文件的 Python 脚本 |
| `工具与脚本/主机模式host` | 文件 | 切换设备为主机模式的配置或脚本 |
| `刷机脚本/` | 目录 | Windows 下一键刷机所需的工具和脚本 |
| `刷机脚本/一键升级补丁.bat` | 文件 | Windows 批处理脚本，自动完成固件刷入流程 |
| `diy-part1.sh` | 文件 | 编译第一阶段自定义脚本，在拉取源码后执行（添加软件源、打补丁等） |
| `diy-part2.sh` | 文件 | 编译第二阶段自定义脚本，在生成默认配置后执行（修改配置、添加文件等） |
| `upstream_history.txt` | 文件 | 上游源码的更新历史记录 |
| `upstream_lock.txt` | 文件 | 锁定上游源码版本的记录文件，用于保证编译可复现 |
| `极简的包名.txt` | 文件 | 常用插件包名速查列表，方便填写编译参数时参考 |
| `.config` | 文件 | 默认编译配置文件，定义全局编译选项 |
